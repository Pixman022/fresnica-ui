...
| Task                                     | Authority                                                                                                        |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Use / compose components (consumer code) | installed package's TypeScript declarations; [skills/fresnica-ui-style/](skills/fresnica-ui-style/SKILL.md)      |
...
1. **Translations** (mirror structure and meaning; tech terms stay English):
    ...
    - `skills/fresnica-ui-style/SKILL.md` ↔ `SKILL.zh-CN.md` (human-review copy; agents read only SKILL.md)
2. **The published skill restates usage** of what the design system defines:
    - component API changes → `skills/fresnica-ui-style/references/components/<category>.md` (props verbatim from source)
...
- Do not add runtime dependencies without an explicit design-system or architecture decision; never disable `preserveModules` / `cssCodeSplit`
