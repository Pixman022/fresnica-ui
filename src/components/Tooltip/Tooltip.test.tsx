...
describe('Tooltip', () => {
    ...
    it('variant=accent 应用强调色类', () => {
        render(
            <Tooltip title="hi" variant="accent">
                <button>btn</button>
            </Tooltip>
        );
        expect(screen.getByRole('tooltip', { hidden: true })).toHaveClass(styles.accent);
    });
    ...
    describe('a11y', () => {
        it('显示时 trigger.aria-describedby 指向 tooltip.id', async () => {
            ...
            expect(tip.id).toMatch(/^Fresnica-tooltip-/);
