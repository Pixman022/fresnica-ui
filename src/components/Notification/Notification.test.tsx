...
const getContainer = (): HTMLElement | null => document.querySelector('[data-Fresnica-notification-root]');
...
describe('Notification', () => {
    ...
    describe('静态方法挂载与渲染', () => {
        ...
        it('compact 形态渲染为轻量状态提示且默认不显示关闭按钮', async () => {
            act(() => {
                Notification.success({ message: '地址已复制', variant: 'compact', duration: 0 });
            });
            await waitForContainer();
            await waitFor(() => {
                const status = screen.getByRole('status');
                expect(status).toHaveClass(styles.compact);
