...
// 默认 4 类型 Lucide 图标，用 currentColor 继承容器颜色
const DEFAULT_ICONS: Record<NotificationItem['type'], React.ReactNode> = {
    success: <Check size={18} strokeWidth={2.5} aria-hidden="true" />,
    info: <Info size={18} strokeWidth={2.5} aria-hidden="true" />,
    warning: <AlertTriangle size={18} strokeWidth={2.5} aria-hidden="true" />,
    error: <X size={18} strokeWidth={2.5} aria-hidden="true" />,
};
...
export const NotificationView: React.FC<NotificationViewProps> = ({ item, onRemove }) => {
    ...
    const cls = classNames(
        ...
        item.variant === 'compact' && styles.compact,
    ...
    const closable = item.closable ?? item.variant !== 'compact';
    const semanticRole = item.onClick ? 'button' : item.type === 'error' ? 'alert' : 'status';
