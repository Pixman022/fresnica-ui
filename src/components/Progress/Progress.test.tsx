...
describe('Progress', () => {
    ...
    describe('animation', () => {
        ...
        it('default fill carries the theme gradient class', () => {
            const { container } = render(<Progress percent={50} />);
            const fillEl = container.querySelector(`.${styles.fill}`) as HTMLElement;
            expect(fillEl).toBeInTheDocument();
            // The CSS module class owns the static theme gradient; jsdom does not resolve CSS rules.
    ...
    describe('inside info fallback for low percent', () => {
        it('shows percent outside fill (track-end) when percent < 18% to keep white text readable', () => {
            ...
            // The fallback should use the semantic track text token instead of white-on-fill
            expect(infoNodes[0]).toHaveStyle({ color: 'var(--Fresnica-text-color-secondary)' });
        });
