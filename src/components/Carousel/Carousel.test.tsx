...
const slides = [<div key="1">资产概览</div>, <div key="2">交易活动</div>, <div key="3">网络设置</div>];
...
describe('Carousel', () => {
    ...
    it('默认展示第一张并提供完整轮播语义', () => {
        render(<Carousel aria-label="网络照片">{slides}</Carousel>);
        expect(screen.getByRole('region', { name: '网络照片' })).toHaveAttribute('aria-roledescription', 'carousel');
    ...
    it('焦点进入后暂停，并可从播放控制显式恢复', async () => {
        ...
        await user.tab();
        expect(screen.getByRole('button', { name: '继续自动播放' }).querySelector('svg')).toBeInTheDocument();
        expect(latestTimer).toBeUndefined();
        expect(onChange).not.toHaveBeenCalled();

        await user.keyboard('{Enter}');
        expect(screen.getByRole('button', { name: '暂停自动播放' }).querySelector('svg')).toBeInTheDocument();
