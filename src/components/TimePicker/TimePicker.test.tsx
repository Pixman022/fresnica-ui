...
describe('TimePicker', () => {
    ...
    describe('展开与选择', () => {
        ...
        it('此刻按钮设置当前时间', async () => {
            ...
            for (let time = Math.floor(before.getTime() / 1000) * 1000; time <= after.getTime(); time += 1000) {
                const current = new Date(time);
                expectedValues.add(
                    `${pad(current.getHours())}:${pad(current.getMinutes())}:${pad(current.getSeconds())}`
                );
            }
            const displayedValue = screen.getByRole('combobox').querySelector(`.${styles.value}`)?.textContent;
            expect(expectedValues).toContain(displayedValue);
        });
    });
