...
describe('Button', () => {
    ...
    it('应用 type / size / 状态相关类', () => {
        render(
            <Button type="primary" size="large" danger block loading>
                OK
            </Button>
        );
    ...
    it('type 全部枚举（primary / default / dashed / text / link / inverse）', () => {
        const types: Array<'primary' | 'default' | 'dashed' | 'text' | 'link' | 'inverse'> = [
            'primary',
            'default',
            'dashed',
            'text',
    ...
    it('inverse 类型使用主题反转操作类', () => {
        render(<Button type="inverse">Swap</Button>);
        expect(screen.getByRole('button', { name: 'Swap' })).toHaveClass(styles['btn-inverse']);
    });
