...
describe('Drawer', () => {
    ...
    it('默认 pushBackground 下沉 body 非固定子元素', async () => {
        ...
        expect(content.style.borderRadius).toBe('var(--Fresnica-border-radius-base)');
        expect(content.style.overflow).toBe('hidden');
        // 回归：portal 包裹器自身不能被下沉（否则 transform 会破坏内部 fixed 面板定位）
        const portalWrapper = document.querySelector('[data-Fresnica-drawer-portal]') as HTMLElement;
    ...
    it('sheet 形态默认从底部弹出并显示拖动指示条', () => {
        render(
            <Drawer open variant="sheet">
                body
            </Drawer>
        );
        const dialog = screen.getByRole('dialog');
        expect(dialog).toHaveClass(styles.panelBottom, styles.variantSheet, styles.safeArea);
        expect(dialog.querySelector(`.${styles.handle}`)).toBeInTheDocument();
    });
