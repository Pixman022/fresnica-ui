...
export type DrawerVariant = 'drawer' | 'sheet';
...
export const Drawer: React.FC<DrawerProps> = ({
    ...
    useEffect(() => {
        ...
        const candidates = Array.from(document.body.children).filter((el): el is HTMLElement => {
            ...
            // 跳过显式标记忽略的元素
            if (el.hasAttribute('data-Fresnica-drawer-ignore')) return false;
            // 跳过本组件 portal 输出（其内部的 mask/panel 是 fixed，
            // 若对其施加 transform 会成为 fixed 后代的包含块，破坏视口定位）
            if (el.hasAttribute('data-Fresnica-drawer-portal')) return false;
        ...
        const rafId = requestAnimationFrame(() => {
            pushed.forEach(({ el }) => {
                ...
                el.style.borderRadius = 'var(--Fresnica-border-radius-base)';
    ...
    const idPrefix = `Fresnica-drawer-${useId().replace(/:/g, '')}`;
    ...
    const panelClass = [
        styles.panel,
        resolvedPlacement === 'left' && styles.panelLeft,
        resolvedPlacement === 'right' && styles.panelRight,
        resolvedPlacement === 'top' && styles.panelTop,
        resolvedPlacement === 'bottom' && styles.panelBottom,
        variant === 'sheet' && styles.variantSheet,
    ...
    const drawerContent = (
        <div data-Fresnica-drawer-portal>
            <div
            ...
            >
                <div
                    ...
                    {...inertProps}
                >
                    {resolvedShowHandle && <span className={styles.handle} aria-hidden="true" />}
                    {title && (
