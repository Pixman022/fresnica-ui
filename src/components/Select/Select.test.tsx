...
describe('Select', () => {
    ...
    it('鼠标移入/移出选项 → 切换 hovered 样式', async () => {
        ...
        expect(weightAfter).toBe('var(--Fresnica-font-weight-bold)');
        expect(weightBefore).not.toBe(weightAfter);
        await user.unhover(banana);
    });

    it('当前选中项仅通过 active 类表达，不渲染装饰圆点', async () => {
        const user = userEvent.setup();
        ...
        expect(banana.children).toHaveLength(0);
    });
    ...
    describe('定位策略', () => {
        ...
        it('bottom 定位 → 菜单保持在触发器下方并限制在可用高度内', async () => {
            mockRect({ top: 200, right: 1900, bottom: 250, width: 160, height: 48 });
            mockViewport(2000, 2000);
            render(<Select options={options} value="" onChange={() => {}} placement="bottom" />);
            const dropdown = await openAndGetDropdown();
            expect(dropdown.style.left).toBe('0px');
            expect(dropdown.style.right).toBe('auto');
            expect(dropdown.style.top).toBe('100%');
            expect(dropdown.style.overflowY).toBe('auto');
            expect(dropdown.style.maxHeight).toBe('1734px');
            expect(dropdown.style.maxWidth).toBe('calc(100vw - 16px)');
        });
        it('bottom alignment stays inside the viewport near the right edge', async () => {
            mockRect({ top: 200, left: 1850, right: 2010, bottom: 250, width: 160, height: 48 });
            mockViewport(2000, 2000);
            render(<Select options={options} value="" onChange={() => {}} placement="bottom" />);
            const dropdown = await openAndGetDropdown();
            expect(dropdown.style.left).toBe('auto');
    ...
    describe('a11y', () => {
        ...
        it('aria-label 透传到 trigger 与 listbox', async () => {
            const user = userEvent.setup();
            render(<Select options={options} value="" onChange={() => {}} aria-label="资产" />);
            const trigger = screen.getByRole('combobox');
            // a11y 契约：aria-label 透传到 trigger 后，可被 toHaveAccessibleName 识别
            expect(trigger).toHaveAccessibleName('资产');
            await user.click(trigger);
            await flushRaf();
            // a11y 契约：aria-label 同时透传到 listbox，屏幕阅读器可读出名字
            expect(screen.getByRole('listbox')).toHaveAccessibleName('资产');
        });
    });
});
