...
export type SelectPlacement = 'auto' | 'bottom';
...
export interface SelectProps {
    ...
    placeholder?: string;
    disabled?: boolean;
    /** 下拉层定位策略；`bottom` 用于需要保持在触发器下方的窄容器。 */
...
export const Select: React.FC<SelectProps> = ({
    ...
    const idPrefix = `Fresnica-select-${useId().replace(/:/g, '')}`;
    ...
    useEffect(() => {
        if (open && wrapperRef.current) {
            ...
            if (placement === 'bottom') {
                const openBelow = spaceBelow >= dropdownHeight || (spaceBelow >= spaceAbove && spaceBelow > 0);
                const availableSpace = Math.max(1, openBelow ? spaceBelow - 16 : spaceAbove - 16);
                // Keep the menu inside the viewport when the trigger sits near the right edge.
                // The final width is content-driven, so use a conservative estimate for alignment
                // and cap the rendered menu in CSS as a second line of protection.
                const estimatedWidth = Math.max(rect.width, 200);
                const alignRight = rect.left + estimatedWidth > viewportWidth - 8;
                newStyle.left = alignRight ? 'auto' : 0;
                newStyle.right = alignRight ? 0 : 'auto';
                newStyle.maxHeight = `${availableSpace}px`;
                newStyle.maxWidth = 'calc(100vw - 16px)';
