...
describe('Tabs', () => {
    ...
    it('支持 segmented/compact 形态、图标和徽标', () => {
        render(
            <Tabs
                variant="segmented"
                size="compact"
                items={[{ key: 'a', label: 'Assets', icon: <span>A</span>, badge: 2, children: 'content' }]}
            />
        );
        expect(screen.getByRole('tablist').closest(`.${styles.tabs}`)).toHaveClass(
            styles['variant-segmented'],
            styles['size-compact']
        );
        expect(screen.getByText('2')).toHaveClass(styles.tabBadge);
    });

    it('disabled tab cannot be selected and keyboard navigation skips it', async () => {
        const onChange = vi.fn();
        render(<Tabs items={[items[0], { ...items[1], disabled: true }, items[2]]} onChange={onChange} />);
        const tabs = screen.getAllByRole('tab');
        expect(tabs[1]).toBeDisabled();
        tabs[0].focus();
        await setup().keyboard('{ArrowRight}');
    ...
    describe('a11y', () => {
        it('tablist / tab / tabpanel 角色 + aria-selected + aria-controls 双向关联', () => {
            render(<Tabs items={items} aria-label="资产" />);
            const tablist = screen.getByRole('tablist');
            expect(tablist).toHaveAttribute('aria-label', '资产');
