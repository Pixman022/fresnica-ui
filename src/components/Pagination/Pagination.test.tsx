...
describe('Pagination', () => {
    ...
    it('应用标准分页根样式', () => {
        render(<Pagination total={50} pageSize={10} />);
        const nav = screen.getByRole('navigation', { name: '分页' });
        expect(nav).toHaveClass(styles.pagination);
    });
    ...
    it('showSizeChanger 支持键盘选择每页条数', () => {
        ...
        fireEvent.click(screen.getByRole('button', { name: '每页 10 条' }));
        fireEvent.keyDown(screen.getByRole('option', { name: '20 条/页' }), { key: ' ' });
        expect(onChange).toHaveBeenCalledWith(1, 20);
        expect(screen.queryByRole('listbox', { name: '选择每页条数' })).not.toBeInTheDocument();
    });
