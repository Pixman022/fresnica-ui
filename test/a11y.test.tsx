 ...
 * --------------------------------------------------------------------------
 * 设计目的：
 *   作为全部组件的"无障碍基线设施"，用 axe-core 跑一轮 WCAG 2.1 AA 自动化
 ...
 *
 * CI 契约：
 *   `npm run ci` 会执行本文件；新增组件时必须同时增加一个典型用法 case。
 *
...
// 不关心动画，所以直接 mock 掉。
vi.mock('@/components/Loading/Fresnica/script.js', () => ({
...
describe('a11y smoke / 全组件 axe-core 自动检查', () => {
    ...
    it('QuickAction', async () => {
        const r = render(<QuickAction label="发送" icon="S" />);
        await expectNoA11yViolations(containerOf(r), 'QuickAction');
    });

    it('BottomNavigation', async () => {
        const r = render(
            <BottomNavigation
                activeKey="assets"
                items={[
        ...
        await expectNoA11yViolations(containerOf(r), 'BottomNavigation');
    });

    it('WalletSwitcher', async () => {
        const r = render(<WalletSwitcher wallet="Main Wallet" address="GABC…WXYZ" />);
        await expectNoA11yViolations(containerOf(r), 'WalletSwitcher');
    });

    it('BackTop', async () => {
        const r = render(<BackTop />);
        await expectNoA11yViolations(containerOf(r), 'BackTop');
    });

    it('Icon (无 aria-label 应有 violation，提示需要补)', async () => {
        // 故意不传 aria-label：第一次跑会暴露"图标裸用没名字"问题
        const r = render(<Icon name="camera" />);
    ...
    it('Collapse (有 question 文案)', async () => {
        const r = render(<Collapse question="Fresnica 支持哪个网络？" answer="当前仅支持 Stellar 网络。" />);
    ...
    it('AddressField (带 aria-label)', async () => {
        const r = render(<AddressField aria-label="Stellar 地址" defaultValue="GABC" />);
        await expectNoA11yViolations(containerOf(r), 'AddressField');
    });

    it('AmountField (带 aria-label)', async () => {
        const r = render(<AmountField aria-label="转账金额" currency="XLM" balance="120 XLM" />);
        await expectNoA11yViolations(containerOf(r), 'AmountField');
    });

    it('Pagination', async () => {
        const r = render(<Pagination total={120} current={2} showSizeChanger showQuickJumper showTotal />);
        await expectNoA11yViolations(containerOf(r), 'Pagination');
    });
    ...
    it('BalanceCard', async () => {
        const r = render(<BalanceCard balance="120.50" currency="XLM" secondaryValue="$14.46" change={1.2} />);
        await expectNoA11yViolations(containerOf(r), 'BalanceCard');
    });

    it('AssetIcon', async () => {
        const r = render(<AssetIcon symbol="XLM" />);
        await expectNoA11yViolations(containerOf(r), 'AssetIcon');
    });

    it('AssetRow', async () => {
        const r = render(<AssetRow name="Stellar Lumens" symbol="XLM" balance="120.50 XLM" fiatValue="$14.46" />);
        await expectNoA11yViolations(containerOf(r), 'AssetRow');
    });

    it('TransactionRow', async () => {
        const r = render(
            <TransactionRow
                title="Sent XLM"
        ...
        await expectNoA11yViolations(containerOf(r), 'TransactionRow');
    });

    it('TransactionStatus', async () => {
        const r = render(<TransactionStatus status="pending" />);
        await expectNoA11yViolations(containerOf(r), 'TransactionStatus');
    });

    it('TransactionDetails', async () => {
        const r = render(
            <TransactionDetails
                assetSymbol="XLM"
                ...
                title="-10 XLM"
                status="success"
                fields={[{ label: 'Network fee', value: '0.00001 XLM' }]}
            />
        );
        await expectNoA11yViolations(containerOf(r), 'TransactionDetails');
    });

    it('FeeSummary', async () => {
        const r = render(
            <FeeSummary
                items={[{ label: 'Network fee', value: '0.00001 XLM' }]}
                total={{ label: 'Total', value: '10.00001 XLM' }}
            />
        );
        await expectNoA11yViolations(containerOf(r), 'FeeSummary');
    });

    it('SwapRoute', async () => {
        const r = render(<SwapRoute from="XLM" via="AQUA" to="USDC" rate="1 XLM = 0.12 USDC" best />);
        await expectNoA11yViolations(containerOf(r), 'SwapRoute');
    });

    it('NetworkBadge', async () => {
        const r = render(<NetworkBadge network="Stellar Mainnet" status="online" />);
        await expectNoA11yViolations(containerOf(r), 'NetworkBadge');
    });
    ...
    it('Typewriter (有 children 文本)', async () => {
        const r = render(<Typewriter>欢迎使用 Fresnica Stellar 钱包</Typewriter>);
    ...
    it('Skeleton (纯装饰)', async () => {
        const r = render(<Skeleton variant="paragraph" rows={3} />);
        await expectNoA11yViolations(containerOf(r), 'Skeleton');
    });

    it('Image (带 alt)', async () => {
        const r = render(<Image src="https://example.com/photo.png" alt="网络风景" width={120} height={120} />);
        await expectNoA11yViolations(containerOf(r), 'Image');
    });

    it('Image (preview 弹层)', async () => {
        const user = userEvent.setup();
        const r = render(<Image src="https://example.com/photo.png" alt="网络风景" preview />);
        await user.click(r.getByRole('button', { name: /网络风景/ }));
