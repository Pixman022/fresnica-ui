...
运行时 CSS 变量是 Fresnica 唯一公开 Token 契约，由 `src/styles/themes/default.less` 输出，组件样式也只使用这套完整名称。使用者可以在 `:root` 或 `[data-theme='dark']` 上覆盖它们；不再维护另一套精简名称。

协议紫语义使用 `--Fresnica-accent-purple-*`。原 `--Fresnica-secondary-*` 兼容别名已在 2.0.0 清理中删除。

需要明确声明“无投影”契约时使用 `--Fresnica-shadow-base`。原 `--Fresnica-shadow-sm` 与 `--Fresnica-shadow-lg` 别名已在 2.0.0 清理中删除。
...
以上完整名称与 `src/styles/themes/default.less` 一致；语义用途与组合规则见同目录的 `design-tokens.md`。
