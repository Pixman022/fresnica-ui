...
组件库交付模块化组件、本地中文字体资源与基于 Lucide 的界面图标。传统的单文件库产物会把这些全部拖进每一个消费应用，不管它 import 的是一个组件还是全部组件。
...
`vite.config.ts` 里的五个插件让这套安排产出一个可用的包 —— 四个定义在该文件内，外加一个第三方插件：
...
- `emit-global-style-entry`（`emitGlobalStyleEntryPlugin`）—— 把全局样式表与全部组件 CSS Module 依次拼接进 `dist/index.css`，支撑 `fresnica-ui/style` 这个子路径。全局样式排在最前，组件规则才能解析到 `:root` 上的自定义属性。资源 URL 从相对源码（`url(../../files/x.svg)`）改写为相对 `dist`（`url("files/x.svg")`），`dist/files/` 中未被引用的文件会被删除。
- `prune-empty-dirs`（`pruneEmptyDirsPlugin`）—— 删除资源重定向在 `dist/` 下留下的空目录树。

`package.json` 补齐契约的另一半：`sideEffects: false`，以及覆盖根入口、`./style`、`./es/*` 的 `exports` 映射。
...
- 构建的正确性依赖插件顺序。`strip-woff-fallback` 必须早于资源收集；聚合插件运行在 `closeBundle`，即 CSS 写盘之后。
