...
本目录记录 `fresnica-ui` 背后承重的技术决策 —— 那些约束后续改动的决定，而非对当前代码的描述。每条记录说明是什么处境逼出了这个选择、选择本身是什么，以及项目为此接受了哪些后果。
...
| [0002](0002-dual-design-token-system.md)  | 双层设计 token   | 需要参与计算的值用 Less 编译期变量，消费者可能重新定制主题的值用 `--Fresnica-*` CSS 自定义属性。            |
...
| [0005](0005-lucide-runtime-icon-system.md) | Lucide 运行时图标体系 | `lucide-react` 是唯一的直接运行时依赖，也是 Fresnica 界面图标的统一来源。                       |
