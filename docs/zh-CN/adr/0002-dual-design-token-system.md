...
组件作者写 Less Modules，需要能参与计算的值 —— 算术、`darken()` / `lighten()`、mixin 参数、媒体查询条件。Less 函数只能作用于编译器已知的值，CSS 自定义属性对它们是不透明的：`darken(var(--Fresnica-primary-color), 10%)` 无法工作。
...
**第一层 —— Less 编译期变量。** `src/styles/variables.less` 是唯一事实来源：色板、中性色、字体排印、间距、圆角、阴影兼容 Token、动效曲线、控件高度，全部以 `@` 前缀声明。它们在编译期被替换为字面量，不存在于运行时。Fresnica 的阴影兼容 Token 固定为 `none`，层级由表面色差和描边建立。
...
**第二层 —— 运行时 CSS 自定义属性。** `src/styles/themes/default.less` 把同一批 token 以 `--Fresnica-*` 属性重新声明在 `:root` 上，每一项都由其 Less 对应项初始化：
...
**编写规则。** 组件样式默认引用 `var(--Fresnica-*)`。只有当值必须在编译期已知时才用 Less 变量：参与计算、颜色函数、mixin 参数或媒体查询。

## 影响

- 消费者在 `:root` 或任意作用域选择器上重新声明 `--Fresnica-*` 即可换主题，无需构建步骤，也无需 fork。
...
- `--Fresnica-*` 这些名字是公开 API。重命名或删除其中之一，对消费者就是破坏性变更，与用它的是哪个组件无关。
