...
- `generateScopedName: 'Fresnica-[local]-[hash:base64:5]'` —— 生成形如 `Fresnica-btn-primary-abc12` 的类名
...
2. **运行时 CSS 自定义属性**（`src/styles/themes/default.less`，`--Fresnica-` 前缀）：`--Fresnica-primary-color`、`--Fresnica-text-color`、`--Fresnica-spacing-sm`、`--Fresnica-font-family`、`--Fresnica-shadow-base` 等。组件 `.module.less` 通过 `var(--Fresnica-*)` 引用，消费者可以覆盖。

组件样式优先用 `var(--Fresnica-*)`，下游主题定制才能生效。只有当值必须参与编译期计算（算术、`darken()`、mixin）时，才用 Less 变量。
...
- 忽略：`dist` / `demo-dist` / `coverage` / `node_modules` / `scripts` / `*.config.{js,ts}` / `**/*.min.js` / `**/*.min.d.ts` / `**/Fresnica/**`
