...
- Bug 反馈与功能建议走 [GitHub Issues](https://github.com/Pixman022/fresnica-ui/issues)。
...
`npm run ci` 是 `format:check` + `audit:tokens` + `check:docs` + `lint` + `test:run` + `test:a11y` + `build`。它必须全绿，PR 才会进入 review。
