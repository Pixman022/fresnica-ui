...
这是新增组件或修改既有组件的端到端流程，只停留在结构层面：单个组件的 props 在 `skills/fresnica-ui-style/references/components/*.md`，单个组件的像素级 spec 在 [../design-system/components/](../design-system/components/)，两者都不要在这里重复一遍。
...
| 改动类型                     | 必须同步                                                                                       |
| ---------------------------- | ---------------------------------------------------------------------------------------------- |
| 新增 / 修改组件 API          | `skills/fresnica-ui-style/references/components/<category>.md` —— props 逐字抄源码        |
