...
单测跑在 jsdom 上的 Vitest 里，无障碍冒烟测试跑在第二份 Vitest 配置里。Token 审计、文档同步、Lint、测试和构建
也都属于 `npm run ci` 的一部分。

`npm run audit:tokens` 是只读检查，会报告 Token 覆盖率、未声明引用、样式中的原始值、旧主题关键词和未使用的公开
契约；它不会自动删除 Token。
