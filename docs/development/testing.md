...
Unit tests run through Vitest in jsdom; accessibility smoke tests run through a second Vitest config. Token audit,
documentation sync, lint, tests and build are all part of `npm run ci`.

`npm run audit:tokens` is read-only. It reports token coverage, undeclared references, raw style values, legacy theme
keywords and unused public contracts; it never removes tokens automatically.
