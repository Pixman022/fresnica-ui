...
- `generateScopedName: 'Fresnica-[local]-[hash:base64:5]'` — produces class names like `Fresnica-btn-primary-abc12`
...
2. **Runtime CSS custom properties** (`src/styles/themes/default.less`, `--Fresnica-` prefix): `--Fresnica-primary-color`,
   `--Fresnica-text-color`, `--Fresnica-spacing-sm`, `--Fresnica-font-family`, `--Fresnica-shadow-base`, … Component
   `.module.less` files reference them through `var(--Fresnica-*)`, and consumers can override them.

Prefer `var(--Fresnica-*)` in component styles so downstream theming works. Reach for Less variables only when the value
