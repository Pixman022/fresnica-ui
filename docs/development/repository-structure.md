...
npm run ci           # format:check + audit:tokens + check:docs + lint + test:run + test:a11y + build
...
src/
  ...
  styles/
    ...
    themes/default.less          # runtime CSS custom properties (--Fresnica-*)
...
skills/
  fresnica-ui-style/        # external-facing skill package (references/components/*.md)
```
