...
- Use [GitHub Issues](https://github.com/Pixman022/fresnica-ui/issues) for bug reports and feature requests.
...
`npm run ci` is `format:check` + `audit:tokens` + `check:docs` + `lint` + `test:run` + `test:a11y` + `build`. It has to be green before a
PR is reviewed.
