...
This directory records the load-bearing technical decisions behind `fresnica-ui` — the ones that constrain future changes rather than describe current code. Each record states the context that forced a choice, the choice itself, and the consequences the project accepted by making it.
...
| [0002](0002-dual-design-token-system.md)   | Dual design token system      | Less compile-time variables for computed values, `--Fresnica-*` CSS custom properties for everything consumers may re-theme.  |
...
| [0005](0005-lucide-runtime-icon-system.md) | Lucide runtime icon system    | Lucide React is the sole direct runtime dependency and the shared source for Fresnica interface icons.                        |
