...
Component authors write Less Modules and need values that participate in computation — arithmetic, `darken()`/`lighten()`, mixin arguments, media query conditions. Less functions operate on values known to the compiler, and a CSS custom property is opaque to them: `darken(var(--Fresnica-primary-color), 10%)` cannot work.
...
**Layer 1 — Less compile-time variables.** `src/styles/variables.less` is the single source of truth: colour palette, neutrals, typography, spacing, border radii, compatibility shadow tokens, motion curves, and control heights, all `@`-prefixed. These are substituted for literals during compilation and do not exist at runtime. Fresnica's compatibility shadow tokens resolve to `none`; hierarchy comes from surface contrast and borders.
...
**Layer 2 — Runtime CSS custom properties.** `src/styles/themes/default.less` re-declares the same tokens on `:root` as `--Fresnica-*` properties, each initialised from its Less counterpart:

```less
:root {
    --Fresnica-primary-color: @primary-color;
    --Fresnica-spacing-sm: @spacing-sm;
    --Fresnica-motion-ease: @motion-ease;
}
```
...
**Authoring rule.** Component styles reference `var(--Fresnica-*)` by default. Reach for a Less variable only when the value must be known at compile time: computation, colour functions, mixin arguments, or media queries.

## Consequences

- Consumers re-theme by redeclaring `--Fresnica-*` properties on `:root` or on any scoped selector, with no build step and no fork.
...
- The `--Fresnica-*` names are public API. Renaming or removing one is a breaking change for consumers, independent of the component that used it.
