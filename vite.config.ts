 ...
 * 相对 dist/ 的形式（url(files/x.svg)），消费者 `import 'fresnica-ui/style'`
 ...
 * 供 `fresnica-ui/style` 子路径一次性引入全量样式。
 * cssCodeSplit 模式下：
...
function emitGlobalStyleEntryPlugin(): Plugin {
    return {
        ...
        closeBundle() {
            ...
            // 扫描覆盖 JS 与 CSS 中的静态资源引用，避免误删组件实际使用的字体或品牌资源。
            ...
            // dist/index.css 也是引用源（fresnica-ui/style 入口）
