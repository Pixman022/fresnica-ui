...
// ============================================
// Skill 介绍 —— fresnica-ui-style
// 章节顺序按阅读动线：是什么 → 怎么用 → 怎么工作 → 用在哪 → 有什么 → 边界
// ============================================
const SkillDemo: React.FC = () => (
    <div className={styles.page}>
        <Section title="fresnica-ui-style" tags={['AI Skill', '可安装']}>
            <p className={styles.lead}>
                一个可安装的编码助手技能，教会 AI 代理（Claude Code、Codex、Cursor 等兼容 SKILL.md 的代理）按Fresnica
                钱包风格搭建 UI：中性明暗表面、绿色主色、Roboto 字体、圆角控件与克制动效。 它只是知识，不是代码 ——
                约束代理「怎么写」，不改变你的工程结构。
            </p>
        ...
        <Section title="目录结构" tags={['Layout']}>
            <ShellCode code={FRESNICA_GUIDE} />
        </Section>

        <Section title="组件目录" tags={['46 Components']}>
            <p className={`${styles.lead} ${styles.leadSpaced}`}>
        ...
        <Section title="设计令牌" tags={['--Fresnica-*']}>
            <p className={`${styles.lead} ${styles.leadSpaced}`}>
                打包样式在 :root 上声明了运行时 CSS 自定义属性（前缀 --Fresnica-*）。下面色板取的是当前页面真实令牌值 ——
