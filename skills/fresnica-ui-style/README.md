...
[fresnica-ui](https://github.com/Pixman022/fresnica-ui) — a React component
library for Fresnica Stellar wallet interfaces.

It covers two scenarios, each with its own entry:

- **React project** — using the `fresnica-ui` npm package: setup, API exploration via
  the package's TypeScript declarations, recipes, theming with `--Fresnica-*` tokens.
...
Or manually: copy this `fresnica-ui-style/` directory into your agent's skills
...
└── references/
    ...
    └── components/              # props references grouped by category
```

Exact design values (every hex/px/keyframe) are deliberately not duplicated into the
skill — it links to the canonical
[design system](https://github.com/Pixman022/fresnica-ui/network/main/docs/design-system)
in the repository.
