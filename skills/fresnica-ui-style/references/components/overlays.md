...
interface ModalProps {
    ...
    typewriter?: boolean; // default false — opt in when the demo needs a typewriter presentation
}
```
...
>
    Proceed to delete this Fresnica?
</Modal>;
```

Notes:

- Modal uses a standard token-driven rounded surface; no custom clipping or decorative artwork is required.
...
```tsx
const [open, setOpen] = useState(false);
<Drawer open={open} title="网络设置" onClose={() => setOpen(false)}>
...
type TooltipVariant = 'default' | 'Fresnica';
...
```tsx
<Tooltip title="Save your Fresnica"><Button type="primary">Save</Button></Tooltip>
<Tooltip title="More info" placement="right" trigger="click">
  <Icon name="message-circle" />
</Tooltip>
<Tooltip title="Additional information" variant="accent"><span>?</span></Tooltip>
```

> `children` must be a SINGLE React element capable of receiving event/ref props (do not pass strings or fragments). `variant="accent"` applies the primary accent surface and border.
>
...
Build confirmations with `Modal`, not a separate wrapper:
...
Use `variant="confirm"` for ordinary confirmation and `variant="danger"` for irreversible actions. Keep `onOk` pending while `okLoading` is true, and let Modal restore focus after close.
