...
- **Status colors use Fresnica semantic tokens.** Error, warning, success and validating feedback inherit the corresponding theme-aware token.
...
interface FormItemProps {
    ...
    hasFeedback?: boolean; // default false — show a Lucide X icon on error
...
- **FormItem injects `value` / `onChange`** (or your custom `valuePropName` / `trigger`) into its child via `React.cloneElement`. The child must accept these props — most fresnica-ui inputs do. For checkbox / switch children use `valuePropName="checked"`.
